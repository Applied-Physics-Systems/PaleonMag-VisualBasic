...
Begin VB.Form frmIRMARM 
   ...
   ScaleHeight     =   5265
   ScaleWidth      =   7905
   Begin VB.Frame frameArmDaqController 
      Caption         =   "ARM DAQ Controller"
      Height          =   1455
      Left            =   5160
      TabIndex        =   44
      Top             =   120
   ...
   Begin VB.Frame frameIRM 
      ...
      Top             =   120
      Width           =   2775
      Begin MSCommLib.MSComm MSCommApsIrm 
         Left            =   1920
         Top             =   1800
         _ExtentX        =   1005
         _ExtentY        =   1005
...
Private Const APS_GET_STATUS As String = "PSS"
Private Const APS_DO_PULSE As String = "PET"
Private Const APS_SET_LEVEL As String = "PCA"
Private Const APS_SET_RANGE_HIGH As String = "PCRH"
Private Const APS_SET_RANGE_LOW As String = "PCRL"
Private Const APS_SET_POLARITY_NEGATIVE As String = "PCRN"
Private Const APS_SET_POLARITY_POSITIVE As String = "PCRP"
Private Const APS_DO_ZERO_DISCHARGE As String = "ZERO"
Private Const APS_ZERO_DISCHARGE_COMMAND_REPLY_FROM_DEVICE As String = "Discharged."
Private Const APS_COMPUTER_MODE As String = "CM"
Private Const APS_MANUAL_MODE As String = "MM"
Private Const APS_MAX_WAIT_TIME_FOR_DONE_IN_SECONDS As Integer = 120 '??? //5/10/17
Private Const APS_DONE_STRING As String = "Done."
Private Const APS_GET_RESPONSE_SERIAL_COMM_TIMEOUT_IN_SECONDS As Integer = 121
...
Public Sub FireIRM(ByVal voltage As Double, _
    ...
    If modConfig.IRMSystem = "ASC" Then
        ...
        Exit Sub
    ElseIf modConfig.IRMSystem = "APS" Then
    
        FireAPS_AtCalibratedTarget voltage
        Exit Sub
...
'------------------------------------------------------------------------------------------------------------'
    ...
    ElseIf TargetVoltage = 0 Then
        ...
        Dim bleed_loop_counter As Integer
        bleed_loop_counter = 0
        ...
        Do While readySignals < 3
            ...
            bleed_loop_counter = bleed_loop_counter + 1
            If bleed_loop_counter > 10 Then
                readySignals = 3
            End If
...
Public Function FireIRMAtField(ByVal Gauss As Double) As Double
    ...
    If modConfig.IRMSystem = "APS" Then
      ...
      result = frmADWIN_AF.TrySetRelays_ADwin(IRMAxial, True) '??? 5/10/17

      Sleep 200 'Allow time for relay to switch //5/10/17
...
Private Sub Form_Activate()
    ...
    Else
        ...
        If modConfig.IRMSystem = "APS" Then
            ConnectApsIRM
            SetControlsEnabledDisabledState_ForApsIRM
        Else
...
Public Sub SetControlsEnabledDisabledState_ForApsIRM()
...
Public Function IRMAverageVoltageIn(Optional ByVal Times As Integer = 5) As Double
    Dim i As Integer
    Dim Sum As Double
...
Public Function IRMBleedBelowVoltage(ByVal voltage As Double, _
    ...
    Else
        ...
        'Set readysignal = 0
        readySignals = 0
        Dim trim_loop_counter As Integer
        trim_loop_counter = 0
        ...
        Do While readySignals < 3
            ...
            trim_loop_counter = trim_loop_counter + 1
            If trim_loop_counter > 20 Then
                readySignals = 4
            End If
...
Public Function SetRelaysForIRM() As Boolean
    ...
    ElseIf AFSystem = "ADWIN" Then
        ...
        Then
            ...
            CoilsLocked = False
            SetRelaysForIRM = frmADWIN_AF.TrySetRelays_ADwin(IRMAxial, IRMBackfieldMode)
            CoilsLocked = True
...
Public Sub SendZeroCommand_ToApsIRMDevice_AndWaitForReply()

    frmProgram.StatusBar "Discharging IRM: ", 2
    frmProgram.StatusBar "Sending ZERO command...", 3
   
    SendApsIrmCommand_AndThenWait600msec APS_DO_ZERO_DISCHARGE
    
    frmProgram.StatusBar "Waiting for reply: """ & APS_ZERO_DISCHARGE_COMMAND_REPLY_FROM_DEVICE & """", 3
    
    
    'Set timers to record how long it takes to receive "DONE" from IRM Device
    Dim Start, CurTimer
    CurTimer = Timer
    Start = CurTimer
    
    Dim continue_wait_for_done_loop As Boolean
    continue_wait_for_done_loop = True
    
    Dim my_local_response As String
    
    'Wait for Done signal
    Do While (continue_wait_for_done_loop)
                    
        my_local_response = ""
        my_local_response = Trim(GetApsIrmResponse(APS_GET_RESPONSE_SERIAL_COMM_TIMEOUT_IN_SECONDS))
        
        'modListenAndLog.AppendRS232MessageToLogFile "Wait For Discharge Loop", modConfig.COMPortApsIrm, my_local_response
        
        If InStr(1, my_local_response, APS_ZERO_DISCHARGE_COMMAND_REPLY_FROM_DEVICE, vbBinaryCompare) > 0 Then
            frmProgram.StatusBar APS_ZERO_DISCHARGE_COMMAND_REPLY_FROM_DEVICE, 3
            Exit Do
        End If
        
        lblIRMStatus.Caption = my_local_response
        
        'Check Code Flow Pause and Flow Halted Interrupts
        ...
        If (Start - CurTimer) > 30 Then
        
            frmProgram.StatusBar "Timeout Error", 3
        
            Dim timeout_message As String
            
            timeout_message = _
                      "Over " & str$(APS_MAX_WAIT_TIME_FOR_DONE_IN_SECONDS) & _
                      " seconds have elapsed since the IRM Discharge Command was sent and " & _
                      " no """ & APS_ZERO_DISCHARGE_COMMAND_REPLY_FROM_DEVICE & """ response has been received from the APS IRM Device. " & _
                      vbNewLine & vbNewLine & _
                      "Please come and check the APS IRM Device and the connection between the IRM device " & _
                      "and the RAPID controlling computer." & vbNewLine & vbNewLine & _
                      "Code execution will continue."
                      
            SetCodeLevel CodeYellow
                    
            'Send an email stating that the IRM settings are wrong
            frmSendMail.MailNotification "APS IRM - No """ & APS_ZERO_DISCHARGE_COMMAND_REPLY_FROM_DEVICE & """ Response received", _
                                         timeout_message, _
                                         CodeYellow
                                            
            SetCodeLevel modStatusCode.StatusCodeColorLevelPrior
            
            lblIRMStatus.Caption = "Timeout Error"
...
Public Sub FireAPS_AtCalibratedTarget(ByVal target As Double)

    'Get Status
    Dim irm_status As ApsIrmModeEnum
    Dim target_level As String
    Dim level_in_positive_gauss As Integer
    
    On Error GoTo APS_IRM_ERROR

    If target = 0 Then
    
        frmProgram.StatusBar "Discharging IRM Device", 2
    
        SendZeroCommand_ToApsIRMDevice_AndWaitForReply
    
        'frmProgram.StatusBar "Invalid IRM Level: " & str$(target), 3
        'Sleep 1000
        
        'frmProgram.StatusBar "IRM Step Skipped", 3
        Sleep 1000
        
        frmProgram.StatusBar "", 2
        frmProgram.StatusBar "", 3
        lblIRMStatus.Caption = ""
        
        Exit Sub
    End If

    frmProgram.StatusBar "IRM @ " & Trim(str$(target)) & " " & modConfig.AFUnits, 2

    frmProgram.StatusBar "IRM Config", 3

    SetApsIrmPolarity_FromTargetValue (target)
    level_in_positive_gauss = GetApsIrmLevelInPositiveIntegerGauss(target)
    SetApsIrmRange_FromGaussLevel (level_in_positive_gauss)
    
    frmProgram.StatusBar "IRM Config", 3
    
    If (Not IsInRangeIrmLevel_IfFailSendCodeYellowErrorEmail(level_in_positive_gauss)) Then
        
        Exit Sub
    
    End If

    target_level = Trim(str$(level_in_positive_gauss))
    ...
    frmProgram.StatusBar "Set Level", 3
    
    'Set IRM Field Level
    'target_level = Chr$(32) + target_level '???
    SendApsIrmCommand_AndThenWait600msec (APS_SET_LEVEL & target_level) '???
    ...
    frmProgram.StatusBar "Fire IRM", 3
    
    'Tell APS IRM to execute pulse
    SendApsIrmCommand_AndThenWait600msec APS_DO_PULSE
    ...
    Do While (continue_wait_for_done_loop)
        ...
        'modListenAndLog.AppendRS232MessageToLogFile "Wait For Done Loop", modConfig.COMPortApsIrm, my_local_response
        
        If InStr(1, my_local_response, APS_DONE_STRING, vbBinaryCompare) > 0 Then
            frmProgram.StatusBar "Done.", 3
            Exit Do
        End If
        
        'If my_local_response = APS_DONE_STRING Then
            'frmProgram.StatusBar "Done.", 3
            'Exit Do
        'End If
        ...
        If (Start - CurTimer) > 30 Then
            ...
            timeout_message = _
                      "Over " & str$(APS_MAX_WAIT_TIME_FOR_DONE_IN_SECONDS) & _
                      " seconds have elapsed since the IRM Fire Command was sent " & _
                      "for an IRM Pulse at " & str$(target) & " " & modConfig.AFUnits & " and " & _
                      " no """ & APS_DONE_STRING & """ response has been received from the APS IRM Device. " & _
                      vbNewLine & vbNewLine & _
                      "Please come and check the APS IRM Device and the connection between the IRM device " & _
                      "and the RAPID controlling computer." & vbNewLine & vbNewLine & _
                      "Code execution will continue."
            ...
            'Send an email stating that the IRM settings are wrong
            frmSendMail.MailNotification "APS IRM - No ""DONE"" Response received", _
                                         timeout_message, _
                                         CodeYellow
    ...
    'Pause for 500 miliseconds so the user can briefly see the status changes
    Sleep 500
...
APS_IRM_ERROR:
    ...
    ErrorMessage = "Unexpected error occurred during APS IRM Pulse to target level: " & _
                   str$(target) & vbNewLine & vbNewLine & _
                   "Error Details: " & vbNewLine & _
                   "(" & Trim(str$(Err.number)) & ") " & _
                   Err.Description
            
    'Send an email stating that the IRM settings are wrong
    frmSendMail.MailNotification "APS IRM ERROR!", _
                                 ErrorMessage, _
                                 CodeRed, _
                                 True
                                 
    MsgBox ErrorMessage, , "APS IRM ERROR!"
    
    SetCodeLevel modStatusCode.StatusCodeColorLevelPrior

End Sub

Public Sub SetApsIrmRange_FromGaussLevel(ByVal level_in_positive_gauss As Integer)

    If (modConfig.ApsIrm_DoRangeChange And _
        level_in_positive_gauss > modConfig.ApsIrm_RangeChangeLevel) Then
        
        frmProgram.StatusBar "Set Range High", 3
        
        SendApsIrmCommand_AndThenWait600msec APS_SET_RANGE_HIGH
        
    Else
        
        frmProgram.StatusBar "Set Range Low", 3
        
        SendApsIrmCommand_AndThenWait600msec APS_SET_RANGE_LOW
        
    End If

End Sub

Public Function GetApsIrmLevelInPositiveIntegerGauss(ByVal target_value As Double) As Integer

    GetApsIrmLevelInPositiveIntegerGauss = Int(Round(Abs(target_value), 0))

End Function

Public Sub SetApsIrmPolarity_FromTargetValue(ByVal target_value As Double)

    If (target_value < 0) Then
        SetApsIrmPolarity_ToNegative
    Else
        SetApsIrmPolarity_ToPositive
    End If

End Sub

Public Sub SetApsIrmPolarity_ToNegative()

    frmProgram.StatusBar "Set Polarity Neg.", 3

    SendApsIrmCommand_AndThenWait600msec APS_SET_POLARITY_NEGATIVE
    
End Sub

Public Sub SetApsIrmPolarity_ToPositive()

    frmProgram.StatusBar "Set Polarity Pos.", 3

    SendApsIrmCommand_AndThenWait600msec APS_SET_POLARITY_POSITIVE
    
End Sub


Public Function IsInRangeIrmLevel_IfFailSendCodeYellowErrorEmail(ByVal irm_level_in_positive_integer_gauss As Integer) As Boolean

        Dim error_message As String
        
        'Default return value is False
        IsInRangeIrmLevel_IfFailSendCodeYellowErrorEmail = False
            
        If (irm_level_in_positive_integer_gauss > modConfig.PulseAxialMax) Then GoTo Is_Above_Max_Allowed_Field
        If (irm_level_in_positive_integer_gauss > modConfig.PulseAxialMax) Then GoTo Is_Below_Min_Allowed_Field
            
        IsInRangeIrmLevel_IfFailSendCodeYellowErrorEmail = True
        
        Exit Function

Is_Above_Max_Allowed_Field:

    error_message = _
        "Target IRM Peak Field (" & Trim(str$(irm_level_in_positive_integer_gauss)) & _
        " " & modConfig.AFUnits & ") is above the currently set value for the Maximum IRM Peak Field (" & _
        Trim(str$(modConfig.PulseAxialMax)) & _
        " " & modConfig.AFUnits & ")." & _
        vbNewLine & vbNewLine & _
        "Code execution will continue."
        
    GoTo Send_Code_Yellow_Error

Is_Below_Min_Allowed_Field:

    error_message = _
        "Target IRM Peak Field (" & Trim(str$(irm_level_in_positive_integer_gauss)) & _
        " " & modConfig.AFUnits & ") is below the currently set value for the Minimum IRM Peak Field (" & _
        Trim(str$(modConfig.PulseAxialMin)) & _
        " " & modConfig.AFUnits & ")." & _
        vbNewLine & vbNewLine & _
        "Code execution will continue."
        
Send_Code_Yellow_Error:
    
        SetCodeLevel CodeYellow
                
        'Send an email stating that the IRM settings are wrong
        frmSendMail.MailNotification "APS IRM - Target Field Out of Range", _
                                     error_message, _
                                     CodeYellow
...
Public Function GetApsIrmStatus() As ApsIrmModeEnum

    SendApsIrmCommand_AndThenWait600msec (APS_GET_STATUS)
    
    modProg.DelayTime 0.3
    
    Dim Response As String
    Response = GetApsIrmResponse
  
     Dim CM As String
     CM = "CM"
         
    'If (Response = APS_COMPUTER_MODE_CR) Then '???
     If InStr(Response, CM) > 0 Then '???
    
        GetApsIrmStatus = Computer
        
    Else
        
        GetApsIrmStatus = Manual
        
    End If

End Function

Public Sub SendApsIrmCommand_AndThenWait600msec(ByVal command As String)
    If MSCommApsIrm.PortOpen = True Then
        MSCommApsIrm.RTSEnable = True
        MSCommApsIrm.OutBufferCount = 0
        MSCommApsIrm.Output = Chr$(13) + command + Chr(13)
        
        modListenAndLog.AppendRS232MessageToLogFile "Send IRM Cmd", modConfig.COMPortApsIrm, command
        
        lblIRMStatus.Caption = "Out: " & command
    Else
        If Not NOCOMM_MODE Then MsgBox "APS IRM Comm Port Not Open"
    End If
...
Public Function GetApsIrmResponse(Optional ByVal timeout_secs As Double = 1) As String
    Dim Delay As Double
    ...
    If MSCommApsIrm.PortOpen = True Then
        ...
        Do While Right$(inputchar, 1) <> vbCr
            DoEvents
            If MSCommApsIrm.InBufferCount > 0 Then
                inputchar = inputchar + MSCommApsIrm.Input
            End If
            If Timer < Delay Then Delay = Delay - 86400
            If Timer - Delay > timeout_secs Then
                ...
                ErrorMessage = "No response received via Serial Comm from IRM device! " & _
                               "Comm has timed out after: " & str$(Timer - Delay) & " seconds." & _
                               vbNewLine & vbNewLine & _
                               "Machine Execution has been paused. Please come into the " & _
                               "lab to check the status of the IRM device!"
                        
                'Send an email stating that the IRM settings are wrong
                frmSendMail.MailNotification "APS IRM COMM ERROR!!", _
                                             ErrorMessage, _
                                             CodeRed, _
                                             True
                                             
                MsgBox ErrorMessage, , "APS IRM COMM ERROR!!"
            ...
            End If
        Loop
        lblIRMStatus.Caption = "In: " + Left$(inputchar, Len(inputchar) - 1)
    Else
        If Not NOCOMM_MODE Then MsgBox "APS IRM Comm Port Not Open"
    End If
    modListenAndLog.AppendRS232MessageToLogFile "Read IRM", modConfig.COMPortApsIrm, inputchar
    
    GetApsIrmResponse = inputchar
End Function

Private Sub ConnectApsIRM()

    If Me.MSCommApsIrm.PortOpen = False And Not NOCOMM_MODE Then
    
        On Error GoTo ErrorHandler  ' Enable error-handling routine.
        MSCommApsIrm.CommPort = Int(val(modConfig.COMPortApsIrm))
        MSCommApsIrm.Settings = modConfig.COMPortApsIrmSettings
        MSCommApsIrm.SThreshold = 1
        MSCommApsIrm.RThreshold = 0
        MSCommApsIrm.inputlen = 1
        MSCommApsIrm.PortOpen = True
        On Error GoTo 0 ' Turn off error trapping.
        If MSCommApsIrm.PortOpen = True Then
        
        Else
            MSCommApsIrm.PortOpen = False
        End If
...
Private Sub DisconnectApsIrm()

    If (modProg.NOCOMM_MODE) Then
        Exit Sub
    End If
    
    If (Not modConfig.EnableAxialIRM) Then
        Exit Sub
    End If
    
    Me.MSCommApsIrm.PortOpen = False
