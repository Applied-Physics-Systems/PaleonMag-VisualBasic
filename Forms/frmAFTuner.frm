...
Private Sub ApplyCoilParam(ByVal CoilParam As SaveCoilParam, _
    ...
    If AFCoilSystem = AxialCoilSystem Or _
       AFCoilSystem = IRMAxialCoilSystem _
    Then
...
Private Sub cmdStartAutoClipTest_Click()
    ...
    If CoilsLocked Then
        ...
        MsgBox "It looks as though another AF process has locked the AF/IRM Relay position, " & _
               "suggesting that an AF ramp or IRM pulse is currently in the process of running. " & _
               vbNewLine & vbNewLine & _
               "No Clip test will be run until the Coil selection has been unlocked."
    ...
    'Clear the AF error status and reset the error response table to Default responses
...
On Error GoTo Relay_Or_Coil_Settings_Error
    ...
    Select Case ActiveCoilSystem
        ...
        Case AxialCoilSystem
            ...
            If Not frmADWIN_AF.TrySetRelays_ADwin(coil_type.Axial) Then
                ...
                Err.Raise -1, "frmAFTuner.SetupAFAutoTune", _
                      "Attempt to setup Relays for Axial ADwin AF ramp failed."
        ...
        Case TransverseCoilSystem
            ...
            If Not frmADWIN_AF.TrySetRelays_ADwin(coil_type.Transverse) Then
                ...
                Err.Raise -1, "frmAFTuner.SetupAFAutoTune", _
                      "Attempt to setup Relays for Transverse ADwin AF ramp failed."
            
            End If
            
         '??? //3/31/17
        Case IRMAxialCoilSystem
        
            CoilString = "IRMAxial"
        
            If Not frmADWIN_AF.TrySetRelays_ADwin(coil_type.IRMAxial, True) Then 'true = do_irm_backfield
            
                Err.Raise -1, "frmAFTuner.SetupAFAutoTune", _
                      "Attempt to setup Relays for IRM Axial ADwin AF ramp failed."
        ...
        Case Else
            ...
            Err.Raise -1, "frmAFTuner.SetupAFAutoTune", _
                      "Invalid Coil System settings.  " & _
                      "Either the AF Axial or the AF Transverse Coil system must be selected. " & _
                      "Currently, neither AF coil system is selected."
    ...
    If Int(val(Me.txtNumSineFits.text)) <= 0 Then Me.txtNumSineFits.text = "100"
    ...
    If Not frmADWIN_AF.TrySetAllRelaysToDefaultPosition_ADwin Then GoTo SetAllRelays_Down_Error
...
Relay_Or_Coil_Settings_Error:
    ...
    MsgBox "An error occurred while trying to setup this ADwin AF Clip-Test Ramp. " & vbNewLine & vbNewLine & _
           "Error Details: " & vbNewLine & _
           Err.Description, vbCritical, _
           "AF Clip-Test Setup Error"
...
Private Sub doAFAutoTune(ByRef AFData() As Double, _
    ...
    If ActiveCoilSystem = IRMAxialCoilSystem Then CoilString = "IRMAxial" '??? //5/5/17
    ...
    'Count the number of freq steps from Minimum Freq to Max Freq for the scan
        ...
        '??? //4/4/17
    If (FreqStepSize = 0) Then
        FreqStepSize = 1 = 1 'Set to 1 to prevent crash '??? //4/4/17
    ...
    '??? //4/4/17
    If (NumSteps <= 1) Then
        NumSteps = 2 'Set to 2 to prevent crash '??? //4/4/17
...
Private Sub Form_Load()
    ...
    '??? 4/4/17Me.txtFreqStepSize = ""
    
    Me.txtFreqStepSize.text = 1 '??? set to 1 to prevent crashes
...
Private Sub SetupAFAutoTune()
    ...
    If WaveForms Is Nothing Or SystemBoards Is Nothing Then
        ...
        'Tell User somethings wrong
        MsgBox "AF System wave-forms and/or the System Boards collection have not been loaded " & _
                  "properly.  Please check the Paleomag.ini file." & vbNewLine & vbNewLine & _
                  "The AF Auto-tune will not be aborted.", vbCritical, _
                  "Error: AF and DAQ Settings loaded incorrectly or missing!"
    ...
    Select Case ActiveCoilSystem
        ...
        Case IRMAxialCoilSystem
            ...
            If Not frmADWIN_AF.TrySetRelays_ADwin(coil_type.Axial, True) Then  'True = backgroundField '???
...
Relay_Or_Coil_Settings_Error:
    ...
    MsgBox "An error occurred while trying to setup the ADwin AF Auto-Tune process. " & vbNewLine & vbNewLine & _
           "Error Details: " & vbNewLine & _
           Err.Description, vbCritical, _
           "AF Auto Tune Setup Error"
