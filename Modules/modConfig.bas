...
Public DCMotorHomeToTop_StopOnTrue As Boolean   'Boolean parameter
                                                'True means stop moving the up/down motor towards to the home (a.k.a. top, a.k.a. zero) position if
                                                'the value on the the Motor Controller digital input channel is True (logic high)
                                                'False means stop moving the up/down motor towards to the home (a.k.a. top, a.k.a. zero) position if
                                                'the value on the the Motor Controller digital input channel is False (logic low)
...
Public HolderMomentTooHighThreshold As Double   'Moment threshold in emu for the Sample Holder above which an automatic remeasurement of the Sample Holder is triggered
Public EnableHolderMomentTooHighRemeasurements As Boolean   'Enable / Disable boolean flag for remeasuring Sample Holder if Holder moment is above threshold
Public HolderMomentTooHigh_NumAllowedRemeasures As Integer  'Max # of times code will attempt to remeasure the Sample Holder if the Holder moment is above threshold
...
Public AFRelays_UseUpPosition As Boolean    'Boolean setting.
                                            'True means that the AF capacitor banks are wired to the up-terminals of the AF relay switches
                                            'False means that the AF capacitor banks are wired to the down-terminals
...
Global Const IRMAxialCoilSystem = 2 '??? //3/31/17
...
Public COMPortApsIrm As Integer
Public COMPortApsIrmSettings As String

Public ApsIrm_RangeChangeLevel As Integer
Public ApsIrm_DoRangeChange As Boolean
...
Public Sub Config_ReadINISettings()
    ...
    DCMotorHomeToTop_StopOnTrue = CBool(Config_GetFromINI("SteppingMotor", "DCMotorHomeToTop_StopOnTrue", "True", Prog_INIFile))
    ...
    Meascount = 1
    HolderMomentTooHighThreshold = val(Config_GetFromINI("Magnetometry", "HolderMomentTooHighThreshold", "5e-7", Prog_INIFile))
    HolderMomentTooHigh_NumAllowedRemeasures = val(Config_GetFromINI("Magnetometry", "HolderMomentTooHighNbHolderRetry", "5", Prog_INIFile))
    EnableHolderMomentTooHighRemeasurements = False
    ...
    AFRelays_UseUpPosition = CBool(Config_GetFromINI("AF", _
                                                     "AFRelays_UseUpPosition", _
                                                     "True", _
                                                     Prog_INIFile))
    ...
    ApsIrm_RangeChangeLevel = val(Config_GetFromINI("IRMPulse", "ApsIrm_RangeChangeLevel", "10000", Prog_INIFile))
    ApsIrm_DoRangeChange = ("True" = Config_GetFromINI("IRMPulse", "ApsIrm_DoRangeChange", "False", Prog_INIFile))
    ...
    COMPortApsIrm = Config_GetFromINI("COMPorts", "COMPortApsIrm", "9", Prog_INIFile)
    COMPortApsIrmSettings = Config_GetFromINI("COMPorts", "COMPortApsIrmSettings", "9600,N,8,1", Prog_INIFile)
...
Public Sub Config_writeSettingstoINI()
    ...
    Config_SaveSetting "SteppingMotor", "DCMotorHomeToTop_StopOnTrue", str$(DCMotorHomeToTop_StopOnTrue)
    ...
    Config_SaveSetting "AF", "AFRelays_UseUpPosition", Trim(CStr(AFRelays_UseUpPosition))
    Config_SaveSetting "AF", "AFUnits", AFUnits
    ...
    '???
    Config_SaveSetting "IRMPulse", "ApsIrm_RangeChangeLevel", str$(ApsIrm_RangeChangeLevel)
    Config_SaveSetting "IRMPulse", "ApsIrm_DoRangeChange", str$(ApsIrm_DoRangeChange)
    '???
...
'------------------------------------------------------------------------------------------------------------------------
    ...
    Config_SaveSetting "COMPorts", "COMPortApsIrm", str$(COMPortApsIrm)
    ...
    Config_SaveSetting "COMPorts", "COMPortApsIrmSettings", COMPortApsIrmSettings
...
'+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++'
            ...
            ElseIf .CommProtocol = ADWIN_COM Then
            
                'Initialize collection that keeps track of ADwin digital output byte channels
